...
'''
    ...
    抓取"https://steamdb.info/sales/"页面中所有游戏的APP_ID
    抓取到1679个游戏的APP_ID（页面上说有1702个）(2020.12.16)
    2405个（2021.3.11）
...
def main():
    ...
    # 保存数据
    # 保存所有的游戏网址先
    # savepath = ".\\SteamAPP_ID.xls"  # 保存路径：    .\\表示保存在当前路径,如果不写就默认保存在当前路径下，保存的文件名称为SteamSales，格式为.xls




# 在这个传入的url中抓取所有游戏的APP-ID的函数定义：
def getData(url):
